...
;(async() => {
    ...
    const gitUserConfig = config[gitUserName.trim()] || config[gitUserEmail.trim()]
    ...
    if (token) {
        ...
        const log = readFileSync(resolve(process.cwd(), 'src/global.ts')).toString().match(/console\.log\(.*\)/)
        const logReg = new RegExp(log[0]
            .replace(/\./g, '\\.')
            .replace(/\(/g, '\\(')
            .replace(/\)/g, '\\)')
            .replace(/'/g, '.')
            ...
            rules:[
                {
                    ...
                    transform({file, code}): Promise<string | void> | string | void {
                        if (logReg.test(code)) {
                            // 更新打包资源版本
                            return (code as string).replace(logReg, template(log[0])({
                                name:'zhijiasoft',
                                version:v
                            }))
                        }
        ...
        if (suffixQuery === 'y') {
            suffix = '-' + await descriptionInit({
                message:'请输入版本后缀类型，内容禁止含有空字符, 最后以分号(;)结束输入',
            })
        }
        let ref = 'main'
        const refQuery = await descriptionInit({
            message:'是否指定tag分支名称或分支Head？y or n, 默认main分支, 最后以分号(;)结束输入',
        })
        if (refQuery === 'y') {
            ref = await descriptionInit({
        ...
        try {
            ...
            if (e.response.data.message === 'Release already exists') {
                ncol.error(`[Release already exists] 该(${tag_name})版本已存在.`)
                ncol.warn('生产环境最新的前5个Release如下：')
                const list = (await releasesVm.list()).map(({tag_name, name}:any) => ({
                    tag_name,
                    name,
                })).slice(0, 5)
                list.forEach(({tag_name, name}:any) => {
                    ncol.warn(`【Name】：${name} ----------> 【TagName】：${tag_name} `)
                })
