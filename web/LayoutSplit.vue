<template>
    ...
    >
        ...
        <div class="LayoutSplitLine" v-if="sliding" ondrag="" :style="styleLine" ref="line"></div>
...
export default {
    ...
    computed:{
        ...
        styleLeft(){
            const span = this.span && this.span[0] || 0.5;
            if(this.Horizontally){
                return {
                    height:Object.prototype.toString.call(span) === '[object String]' ? span : `calc((100% - ${this.lineWidth}px) * ${span} + ${this.clientX}px)`
                }
            }else{
                return {
                    width:Object.prototype.toString.call(span) === '[object String]' ? span : `calc((100% - ${this.lineWidth}px) * ${span} + ${this.clientX}px)`
                }
            }
        },
        styleRight(){
            const span = this.span && this.span[1] || 0.5;
            if(this.Horizontally){
                return {
                    height:Object.prototype.toString.call(span) === '[object String]' ? span : `calc((100% - ${this.lineWidth}px) * ${span} - ${this.clientX}px)`
                }
